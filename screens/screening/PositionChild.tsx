...
export default function PositionChild() {
    ...
    const permissionDenied = permission?.status === 'denied' || microphonePermission?.status === 'denied';

    const renderContent = () => {
        if (cameraReady) return <CameraView style={StyleSheet.absoluteFillObject} facing="front" />;
